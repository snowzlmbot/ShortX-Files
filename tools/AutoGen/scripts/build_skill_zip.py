...
def build_zip(repo_root: Path, output: Path) -> tuple[str, int]:
    ...
    required = skills_root / "shortx-rule-creator" / "SKILL.md"
    if not required.is_file():
        raise SystemExit(f"missing skill entry: {required}")

    files = [
        path
        for path in sorted(skills_root.rglob("*"), key=lambda item: item.as_posix())
        if path.is_file()
    ...
    if not files:
        raise SystemExit(f"no files found under {skills_root}")
    ...
    with zipfile.ZipFile(temp, "w", compression=zipfile.ZIP_DEFLATED, compresslevel=9) as archive:
        for path in files:
            # Archive the contents of skills/ at the archive root so extraction
            # into the configured private skills directory is deterministic.
